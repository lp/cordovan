class Tester < Shoes
	...
	def load
			Cordovan.lattice(binding,:height => 700,:width => 500, :v_grid => 12, :h_grid => 12,:background => red) do |cordovan|
				cordovan.button("okyyy!!", :v_pos => 8, :h_pos => 8, :v_span => 2, :h_span => 4) {clear; other}
				cordovan.edit_box(:v_pos => 4, :h_pos => 2, :v_span => 4, :h_span => 6)
				cordovan.magical(:v_pos => 10, :h_pos => 6, :v_span => 2, :h_span => 6)
				cordovan.lattice(:v_grid => 4, :h_grid => 4,:v_pos => 9, :h_pos => 2, :v_span => 2, :h_span => 6) do |sub_cordovan|
